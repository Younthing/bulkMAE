...
| ORA、GSEA、原生结果与富集绘图 | [富集分析教程](enrichment-analysis.html) |
| 基因集打分和调控活性 | `score_*()`、`activity_*()` |
| 批次、共表达、去卷积和临床模型 | `adjust_*()`、`coexpr_*()`、`deconv_*()`、`surv_*()` |
| P0 共表达 / 分型（airway）；去卷积在无参考时用 `mae_simulate()` | [共表达模块](coexpression-modules.html)、[分子分型](molecular-subtyping.html)、[免疫去卷积](immune-deconvolution.html) |
