...
载入对象包含 63,677 个基因和 8 个样本。四个 cell line 各有一份 untreated
与一份 treated 样本，因而处理比较不是 8
个互相独立样本的简单两组比较。后面的模型使用 `~ cell + dex`：`cell`
吸收细胞系间的基线差异， `dex` 估计在四对样本中一致的地塞米松效应。
...
每个 cell line 的 untreated/treated 配对结构 {.table}

## 原始计数的文库质控

[`qc_library()`](https://younthing.github.io/bulkMAE/reference/qc_library.md)只计算透明的样本级汇总；绘图函数返回普通
...
![八个 airway
样本的文库规模、检出基因数与零计数比例分面柱状图。](airway-qc-de_files/figure-html/library-qc-1.png)
...
过滤后保留 15,926 / 63,677 个基因（25.0%）。

方差稳定化用于相关性、PCA 和热图等探索性图形。`blind = FALSE`
...
![八个 airway 样本的 VST 表达 Pearson
相关性热图。](airway-qc-de_files/figure-html/correlation-1.png)
...
![airway 样本的 PCA 散点图，按 dex 着色并按 cell line
设定点形。](airway-qc-de_files/figure-html/pca-1.png)
...
![八个 airway 样本的稳健 PCA
距离点图，虚线为筛查阈值。](airway-qc-de_files/figure-html/outliers-1.png)

图 4：基于前五个主成分的稳健距离及 99% 卡方筛查阈值。

稳健距离标记是需要回看原始记录、比对质量和其它 QC
指标的**审查信号**，不是自动删除样本的规则。本流程不会因为该图移除任何样本。
...
在 FDR ≤ 0.05 且绝对效应估计 ≥ 1 的联合阈值下，共有 925 个基因入选，其中
478 个上调、447 个下调。缺失 adjusted p-value 按“不显著”处理。
...
|  | feature_id | symbol | effect | standard_error | p_value | adjusted_p_value |
|:---|:---|:---|---:|---:|---:|---:|
...
按 adjusted p-value 排序的前 10 个基因 {.table}
...
![airway 差异表达 MA 图，横轴为对数尺度平均丰度，纵轴为处理效应，并标注
CRISPLD2。](airway-qc-de_files/figure-html/ma-plot-1.png)
...
![airway 差异表达火山图，蓝色为下调、灰色为不显著、橙红色为上调，并标注
CRISPLD2。](airway-qc-de_files/figure-html/volcano-plot-1.png)

图 6：处理效应与原始 p-value 的火山图；颜色由 FDR 与效应阈值共同决定。

最后选择 adjusted p-value 最小的 30 个基因，并在 VST
尺度上按行标准化。这里必须显式传入 feature
ID；绘图函数不会暗中选择“最显著”基因。
...
![三十个差异最显著基因在八个 airway 样本中的 VST 行 z-score
热图，按处理分面。](airway-qc-de_files/figure-html/heatmap-1.png)
...
数据来自 airway ExperimentData 包，对应 Himes 等人的研究：*RNA-Seq
...
Cells*（PLoS ONE, 2014；PMID 24926665；GEO GSE52778）。数据包说明见
[Bioconductor airway
...
和
[`de_ranks()`](https://younthing.github.io/bulkMAE/reference/de_ranks.md)继续用于
ORA 或 GSEA，请阅读
[富集分析教程](https://younthing.github.io/bulkMAE/articles/enrichment-analysis.md)。
