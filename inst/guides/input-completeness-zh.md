...
| `surv_formula()`、`surv_km()`、`surv_roc()`、`surv_cox()`、`surv_cox_table()`、`surv_cox_univariable()`、`surv_risk_groups()`、`surv_penalized()`、`ml_glmnet()` | A（有条件） | 在真实且对齐的 follow-up、event、outcome 和协变量存在时离线拟合。单队列交叉验证不等于外部验证。合成随访必须在图注中标明。 |
