...
Each tile is one already-computed compound/cell score. The caller
supplies the result rows to display; the constructor does not pick "top
hits" from a larger unfiltered table unless `compounds` or `cells` is
passed.
...
- cells:
  ...
  Optional cell-line identifiers used to subset columns.
...
- cluster_rows, cluster_columns:
  ...
  Reorder compounds or cells by hierarchical clustering of the displayed
  scores. Dendrograms are not drawn.
